...
import { Message, LLMRequest, LLMResponse, EnvCheckResponse, PromptParams } from './types';
...
function buildPrompt({ theme, history, speaker }: PromptParams): string {
    ...
    const recentHistory = (history || [])
        .filter((m): m is Message => m && (m.role === 'user' || m.role === 'chatgpt' || m.role === 'gemini'))
        .slice(-10)
        .map((m: Message) => {
...
async function askChatGPT({ theme, history }: { theme: string; history: Message[] }): Promise<string> {
    if (!openai) {
...
async function askGemini({ theme, history }: { theme: string; history: Message[] }): Promise<string> {
    if (!genAI) {
...
ipcMain.handle('env:check', async (): Promise<EnvCheckResponse> => {
    return {
...
ipcMain.handle('llm:ask', async (_evt, { provider, theme, history }: LLMRequest): Promise<LLMResponse> => {
    try {
        if (provider === 'chatgpt') {
